...
SOURCES = $(SRC)/action.c $(SRC)/actions.c $(SRC)/arity.c $(SRC)/array.c $(SRC)/calculator.c $(SRC)/call.c $(SRC)/interpreter.c $(SRC)/printer.c $(SRC)/project.c $(SRC)/resolution.c $(SRC)/state.c $(SRC)/tracer.c
...
# Compiler invocations can get too long.
$(BIN)/%: $(OBJECTS) $(OBJ)/%.o
	@ $(ECHO) $(CC) ... -o $@ $^
	@ $(CC) $(CFLAGS) -o $@ $^
